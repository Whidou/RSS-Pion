...
public class ArticleDAO {
	...
	static {
		...
		ArticleDAO.fieldsOfTheAssociatedTable.add(new String[] { "idEnclosure",
				"INTEGER" });
		ArticleDAO.fieldsOfTheAssociatedTable.add(new String[] { "idFlux",
				"INTEGER" });
	...
	public static void deleteArticleFromDB(final Article article) {
		...
		// Suppression du flux
		Constants.sqlHandler.delete(ArticleDAO.nameOfTheAssociatedTable,
				"id=?", new String[] { article.getId().toString() });
	}
	...
	public static Article getArticleFromDB(final Long id) {
		...
		c = Constants.sqlHandler.query(ArticleDAO.nameOfTheAssociatedTable,
				null, "id=?", new String[] { id.toString() }, null, null, null,
				null);

		articles = ArticleDAO.getArticlesFromDB(c);
	...
	protected static List<Article> getArticlesFromDB(final Cursor c) {
		...
		do {
			...
			article.setAuthor(c.getString(c.getColumnIndex("author")));
			article.setComments(c.getString(c.getColumnIndex("comments")));
			article.setDescription(c.getString(c.getColumnIndex("description")));
			article.setIdFlux(c.getLong(c.getColumnIndex("idFlux")));
			article.setIsRead((c.getInt(c.getColumnIndex("isRead")) != 0) ? true
					: false);
			article.setLink(c.getString(c.getColumnIndex("link")));
			article.setPubDate(c.getLong(c.getColumnIndex("pubDate")));
			article.setSource(c.getString(c.getColumnIndex("source")));
			...
			// Obtention des objets associés
			article.setEnclosure(EnclosureDAO.getEnclosureFromDB(c.getLong(c
					.getColumnIndex("idEnclosure"))));
			article.setGuid(GuidDAO.getGuidFromDB(c.getLong(c
	...
	public static List<Article> getArticlesFromDB(final Long id) {
		...
		c = Constants.sqlHandler.query(ArticleDAO.nameOfTheAssociatedTable,
				null, "idFlux=?", new String[] { id.toString() }, null, null,
				null, null);

		return ArticleDAO.getArticlesFromDB(c);
	}
	...
	public static Long insertArticleIntoDB(final Article article) {
		...
		if (idArticle == null) {
			...
			// Insertion du article
			idArticle = Constants.sqlHandler.insert(
					ArticleDAO.nameOfTheAssociatedTable, "idFlux", valuesMap);
		...
		// Insertion des objets associés
		valuesMap.put("idEnclosure",
				EnclosureDAO.insertEnclosureIntoDB(article.getEnclosure()));
		valuesMap.put("idGuid", GuidDAO.insertGuidIntoDB(article.getGuid()));

		// Update de l'article
		Constants.sqlHandler.update(ArticleDAO.nameOfTheAssociatedTable,
				valuesMap, "id=?", new String[] { idArticle.toString() });
	 ...
	 * @return ID des entrées en BDD
	 ******************************************************************************/
	public static List<Long> insertArticlesIntoDB(final List<Article> articles) {

		final List<Long> ids = new ArrayList<Long>();

		for (final Article article : articles) {
			ids.add(ArticleDAO.insertArticleIntoDB(article));
		}
	...
	/***************************************************************************/
	/**
	 * Met à jour le champs isRead de l'article de la BDD.
	 * 
	 * @param article Article à mettre à jour
	 ******************************************************************************/
	public static void updateIsReadArticleFromDB(final Article article,
			final Integer value) {
		if (article == null) {
			return;
		}
		...
		c.put("isRead", value);
		Constants.sqlHandler.update(ArticleDAO.nameOfTheAssociatedTable, c,
				"id=?", new String[] { article.getId().toString() });
	}

	/***************************************************************************/
	/**
	 * Met à jour les champs isRead des articles de la BDD.
	 * 
	 * @param articles Liste des articles à mettre à jour
	 ******************************************************************************/
	public static void updateIsReadArticlesFromDB(final List<Article> articles,
			final Integer value) {
		for (final Article article : articles) {
			ArticleDAO.updateIsReadArticleFromDB(article, value);
		}
	}
}
